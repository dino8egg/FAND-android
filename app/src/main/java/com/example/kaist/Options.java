...
public class Options extends AppCompatActivity {
    ...
    public String update_text(int input){
        ...
        ret += ("Registered people: "+Integer.toString(voice)+"\n");
        if(train == 0) ret += ("Train: No train data\n");
        if(train == 1) ret += ("Train: Now voice training\n");
        if(train == 2) ret += ("Train: Voice training possible\n");
        if(train == 3) ret += ("Train: Voice training complete\n");
        if(test == 0) ret += ("Test: No model\n");
        if(test == 1) ret += ("Test: Now voice training\n");
        if(test == 2) ret += ("Test: Voice test possible\n");
        return ret;
    }
}
